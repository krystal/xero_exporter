  ...
  context '#create_invoice' do
    it 'should create an invoice if there are lines for one' do
      executor, state = create_executor do |e, api|
        ...
        expect(api).to receive(:post).with('TaxRates', anything).at_least(:once) do |path, params|
          {
        ...
        # in this result.
        expect(api).to receive(:get).with('Contacts', anything).and_return({ 'Contacts' => [{ 'ContactID' => 'abcdef' }] })

        # We'll expect that the invoice will be posted to the Xero API.
        expect(api).to receive(:post).with('Invoices', anything) do |path, params|
